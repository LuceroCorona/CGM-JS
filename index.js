...
const CrearCards = data => {
    data.forEach(item => {
        ...
        templateCard.querySelector('h3').textContent = "$"      
        templateCard.querySelector('h2').textContent = item.precio
...
const setCarrito = item => {
    const producto = {
        ...
        precio: item.querySelector('h2').textContent,
    ...
    botonFinalizar.addEventListener('click', () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
              Swal.fire('¡Listo! Ahora sólo queda ir al carrito', '<a href="">Ir al carrito</a>', 'success',Swal.disableButtons())
